...
def test_forecast():
    ...
    for margs in method_arg_list:
        ...
        for p in [progress, reload_progress]:
            ...
            # Make sure that no exceptions were raised in try block.
            assert np.all([
                isinstance(x_do[0], np.ndarray) for x_do in p["X_do_preds"]])
...
def test_forecast_opt_all():
    ...
    for margs in method_arg_list:
        ...
        for p in [progress, reload_progress]:
            ...
            # Make sure no exceptions were raised by try-block.
            assert np.all([
                isinstance(x_do[0], np.ndarray) for x_do in p["X_do_preds"]])
